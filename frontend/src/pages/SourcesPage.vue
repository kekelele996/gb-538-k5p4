  ...
  if (toState === 'retired') {
    try {
      await ElMessageBox.confirm(
        `废止 ${selected.value.source_code} V${selected.value.version} 后，冻结了该谱版本的未确认归因运行将在同一事务内自动转为“已失效”，并记录触发来源；已确认结果不受影响。`,
        '废止声源谱将级联失效归因结果',
        { confirmButtonText: '废止并失效相关运行', cancelButtonText: '取消', type: 'warning' },
      )
    } catch {
      return
    }
  }
  try { selected.value = await store.transition(selected.value, toState); ElMessage.success(toState === 'active' ? '声源谱已启用' : '声源谱已废止，相关未确认归因运行已失效') }
